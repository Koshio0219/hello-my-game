...
public class Platforming : MonoBehaviour
{
    ...
    public void GroundCheck()
    {
        ...
        if (Physics.SphereCast(transform.position + origin, radius, Vector3.down, out hit, rayDistance, m_layerMask))
        {
            ...
            else if (LayerMask.LayerToName(hit.collider.gameObject.layer) == "Ground" || hit.collider.transform.CompareTag("Center"))
            {
                ...
                //親にcenterがいるか調べる
                GameObject p = plat;
                while (LayerMask.LayerToName(p.layer) == "Ground" && p.transform.parent != null)
                {
